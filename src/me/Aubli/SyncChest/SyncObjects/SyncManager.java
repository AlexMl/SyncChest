...
public class SyncManager {
	...
	this.mainChests = new ArrayList<MainChest>();
	this.relatedChests = new ArrayList<RelatedChest>();
	this.syncHoppers = new ArrayList<SyncHopper>();
	...
	File rFolder = new File(this.relatedChestPath);
	...
	File hFolder = new File(this.hopperPath);
	...
	MainChest[] chests = new MainChest[this.mainChests.size()];
	
	for (int i = 0; i < this.mainChests.size(); i++) {
	    chests[i] = this.mainChests.get(i);
	}
	return chests;
    }
    
    public RelatedChest[] getRelatedChests() {
	RelatedChest[] chests = new RelatedChest[this.relatedChests.size()];
	
	for (int i = 0; i < this.relatedChests.size(); i++) {
	    chests[i] = this.relatedChests.get(i);
	}
	return chests;
    }
    
    public SyncHopper[] getHoppers() {
	SyncHopper[] hopper = new SyncHopper[this.syncHoppers.size()];
	
	for (int i = 0; i < this.syncHoppers.size(); i++) {
	    hopper[i] = this.syncHoppers.get(i);
	}
	return hopper;
    }
	...
	for (int i = 0; i < getChests(chest).length; i++) {
		...
		if (mChest.getY() == loc.getBlockY()) {
			...
			if (mChest.getZ() + 1 == loc.getBlockZ() || mChest.getZ() - 1 == loc.getBlockZ()) {
				...
				MainChest newMChest = new MainChest(this.mainChestPath, ChestType.MAIN, loc, ChestID, true);
				newMChest.setDoubleChest(true, mChest.getID());
				mChest.setDoubleChest(true, ChestID);
				this.mainChests.add(newMChest);
		...
		if (rChest.getY() == loc.getBlockY()) {
			...
			if (rChest.getZ() + 1 == loc.getBlockZ() || rChest.getZ() - 1 == loc.getBlockZ()) {
				...
				// Doppelkiste gefunden
				RelatedChest newRChest = new RelatedChest(this.relatedChestPath, ChestType.RELATED, loc, ChestID, true);
				newRChest.setDoubleChest(true, rChest.getID());
				rChest.setDoubleChest(true, ChestID);
				this.relatedChests.add(newRChest);
	...
	if (chest == ChestType.MAIN) {
	    MainChest m = new MainChest(this.mainChestPath, ChestType.MAIN, loc, ChestID, false);
	    this.mainChests.add(m);
	} else if (chest == ChestType.RELATED) {
	    RelatedChest r = new RelatedChest(this.relatedChestPath, ChestType.RELATED, loc, ChestID, false);
	    this.relatedChests.add(r);
	}
	return true;
    }
	...
	if (chest == ChestType.RELATED) {
		...
		rChest.unLink();
		rChest.delete();
		this.relatedChests.remove(rChest);
		return true;
	    } else {
		return false;
	    }
	...
	SyncHopper h = new SyncHopper(this.hopperPath, HopperID, hopperLoc, sourceLoc, destLoc);
	this.syncHoppers.add(h);
    }
